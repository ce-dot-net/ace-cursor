...
### Security
- **Zero known vulnerabilities** (`npm audit` → `found 0 vulnerabilities`). Forced `serialize-javascript: ^7.0.5` via `package.json` `overrides` field — the last 2 high-severity CVEs (RCE via `RegExp.flags` + DoS via crafted array-likes) were pulled in transitively by `mocha@10` which pins `serialize-javascript@^6.0.2`. The override satisfies the security advisory without downgrading mocha.
...
### Changed
- **Bumped `@ace-sdk/core` to ^2.18.1** — picks up four minor versions of upstream improvements since the v0.2.69 pin to 2.14.0. Smoke-tested against unit suite (323/323 pass) and production esbuild bundle.

## [0.2.71] - 2026-05-04

### Changed
- **New brand mark for Marketplace listing** — replaced the placeholder `icon.png` (1 KB) with the new ACE brand mark (256×256 PNG, mint Λ-wedge with horizontal trace). Covers both VS Code Marketplace and Open VSX listing — both stores read `package.json:icon` from the same VSIX.
...
### Changed
- **Workspace opt-in** — ACE no longer auto-initializes in every Cursor workspace. New workspaces get a prompt: "Enable ACE pattern learning?" with [Yes, enable] / [Not now] / [Never for this workspace]. No MCP server, hooks, rules, or scripts are created until the user opts in. Previously initialized workspaces continue working normally. "Never" stores `ace.enabled: false` in workspace settings. Status bar shows disabled state with link to configure.
...
### Fixed
- **Rules: replace `ace_get_playbook` with `ace_search`** — The continuous-search rule said `Start task → ace_get_playbook()` causing the AI to dump 750KB+ of all patterns instead of searching for 5-10 relevant ones. All rules, help commands, and diagnostics now consistently direct the AI to use `ace_search`.
...
### Added
- **`ACE_CLIENT_ID=cursor` env var** — MCP subprocess now sends `X-ACE-Client: cursor` header via `ACE_CLIENT_ID` env var (ace-sdk 2.12.0+). Enables per-extension analytics tracking in ACE server BigQuery.
